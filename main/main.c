...
static RTC_DATA_ATTR struct timeval
    main_sleep_enter_time;  // For general timer wakeups if any, distinct from
                            // ADXL
static RTC_DATA_ATTR uint8_t state_flags = 0;
static RTC_DATA_ATTR bool rtc_is_adxl_inactive =
    true;  // True if system considers ADXL state as inactive
           // This is passed to and updated by adxl_sleep_manager
...
void app_main(void) {
    ...
    adxl_sm_init_i2c_and_adxl();
    adxl_sm_setup_interrupts();
    /* End of Drivers */
        ...
        switch (state) {
            case wakeup: {
                app_wake_event_t app_event =
                    adxl_sm_determine_wake_state(&rtc_is_adxl_inactive);
                switch (app_event) {
